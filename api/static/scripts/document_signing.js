...
const toBase64 = file => new Promise((resolve, reject) => {
    ...
    reader.onload = () => resolve(Base64.encode(reader.result));
...
console.log('PLUM PLUM PLUM')

async function connectAndSign(submitEvent) {
    ...
    let blob = new Blob([base64ToArrayBuffer(base64EncodedSignature)], {'type': 'text/plain'})
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = 'signature.cms'
...
document.getElementById('myForm1').onsubmit = connectAndSign

console.log('PLUM PLUM PLUM 2')
